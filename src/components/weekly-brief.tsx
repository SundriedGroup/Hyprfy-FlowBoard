...
function PlanPreview({ plan }: { plan: WeeklyPlan }) {
  ...
  return <section className="week-plan-preview">
    ...
    {plan.brandDirection && <div className="brand-direction-strip"><article><span>Mission anchor</span><p>{plan.brandDirection.missionAnchor}</p></article><article><span>Audience focus</span><p>{plan.brandDirection.audienceFocus}</p></article><article><span>Objective</span><p>{plan.brandDirection.strategicObjective}</p></article><article><span>Voice</span><p>{plan.brandDirection.voiceDirection}</p></article></div>}
    ...
    <div className="week-plan-days">{weekdayKeys.map((key) => {
      ...
      return <article key={key}><header><div><span>{key.slice(0, 3)}</span><strong>{day.theme}</strong></div><em>{recommendationLabel(day.recommendation)}</em></header><p>{day.focus}</p>{day.contentItems.length > 0 ? <div className="planned-content-list">{day.contentItems.map((item) => <div key={`${item.title}-${item.primaryChannel}`}><span>{item.primaryChannel === "X" ? "X / Twitter" : item.primaryChannel}</span><strong>{item.title}</strong><small>“{item.hook}”</small>{item.contentPillar && <small className="content-pillar">Pillar · {item.contentPillar}</small>}{item.brandFit && <small className="brand-fit">{item.brandFit}</small>}</div>)}</div> : <small className="no-post-day">No scheduled post · {day.storyOpportunity}</small>}<footer>{day.todoItems.length} tasks · {day.contentItems.reduce((total, item) => total + item.durationMinutes, 0) + day.todoItems.reduce((total, item) => total + item.durationMinutes, 0)} min</footer></article>;
    })}</div>
  </section>;
}
