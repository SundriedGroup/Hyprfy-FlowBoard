...
export function Dashboard({ dates, days, items, onOpenFlowboard }: DashboardProps) {
  ...
  const weekItems = useMemo(() => items.filter((item) => item.item_type !== "vlog" && item.day && dateKeys.has(item.day)), [dateKeys, items]);
  ...
  const publishReady = weekItems.filter((item) => item.status === "done").length;
        ...
        <section className="dashboard-panel week-dashboard">
          ...
          <div className="week-summary-list">
            {dates.map((date) => {
              ...
              const dayItems = items.filter((item) => item.item_type !== "vlog" && item.day === key);
